...
{
    // clear output dir
    if (system("rm -rf ./mega_sweep_output") != 0)
        std::cout << "Failed to \"rm -rf ./mega_sweep_output\"\n";
    if (system("mkdir ./mega_sweep_output") != 0)
        std::cout << "Failed to \"rm -rf ./mega_sweep_output\"\n";
    const int N_POINTS{25'000};
    const int N_POINTS_SUB_DIV{1'000};

    std::string const vpp_error{"1.2287040E-3"};
    std::string const time_error{"50E-9"}; // from elvis specs
    std::string output_data_block_filename{"./mega_sweep_output/output_data_block_"};
    std::string output_param_filename{"./mega_sweep_output/output_param_"};
    ...
    std::ifstream file_count{"./mega_sweep_input/ampl_data"};
    ...
    std::ifstream file_sweep_in{"./mega_sweep_input/sweep_data"};
    if (!file_sweep_in.is_open())
        std::cout << "Failed to open sweep_data" << std::endl;
    std::ifstream file_ampl_in{"./mega_sweep_input/ampl_data"};
    if (!file_ampl_in.is_open())
        std::cout << "Failed to open ampl_data" << std::endl;
    std::ifstream file_phase_in{"./mega_sweep_input/phase_data"};
    ...
    for (int block_n = 0; block_n != N_BLOCKS; ++block_n)
    {
        ...
        for (int sub_block_n = 0; sub_block_n != N_POINTS / N_POINTS_SUB_DIV; ++sub_block_n)
        {
            std::ofstream file_data_block_out{output_data_block_filename + std::to_string(block_n) + "_" + std::to_string(sub_block_n) + ".txt", std::ios::out | std::ios::trunc};
            ...
            for (int i = 1; i != N_POINTS_SUB_DIV; ++i)
            {
                std::string time;
                std::string V_s;
                std::string V_w;
            ...
            std::cout << "\t Done SUB_BLOCK " << sub_block_n << '\n';
